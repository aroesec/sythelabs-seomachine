...
Meta Description: "42,900+ exposed ClawdBot instances. Critical RCE vulnerabilities. Plaintext credential storage. Get the full risk breakdown and enterprise protection steps."
...
Security researchers have identified over 42,900 exposed ClawdBot instances across 82 countries. Of those, 15,200 are directly exploitable through remote code execution. ClawdBot (now called OpenClaw) became the fastest-growing open-source project in GitHub history, accumulating 145,000 stars and 720,000 weekly downloads in just a few months. The speed of adoption has far outpaced the security maturity of the project.
...
The vulnerabilities span multiple categories, from critical CVEs to fundamental architectural design flaws. Here is what security teams need to know.

### Authentication bypass and network exposure

ClawdBot's default configuration binds to `0.0.0.0:18789`, exposing the service to all network interfaces rather than restricting it to localhost. Additionally, authentication is disabled by default. WebSocket connections lack origin verification. Localhost connections receive implicit trust, which creates bypass opportunities when the tool runs behind reverse proxies.

Security researcher Jamieson O'Reilly of Dvuln examined eight publicly exposed instances and found zero authentication on any of them. The [SecurityScorecard STRIKE team](https://securityscorecard.com/blog/beyond-the-hype-moltbots-real-risk-is-exposed-infrastructure-not-ai-superintelligence/) later identified 42,900+ exposed instances, with 93% exhibiting critical authentication bypass flaws.
...
| CVE-2026-25157 | 7.8 (High) | SSH command injection on macOS |
...
In February 2026, researchers disclosed additional vulnerabilities including CVE-2026-26323 (maintainer compromise), CVE-2026-26329 (token replay), and CVE-2026-26327 (discovery spoofing). Endor Labs disclosed six more vulnerabilities on February 18, 2026, covering SSRF, missing authentication, and path traversal.
...
Separately, fake VS Code extensions labeled "Clawdbot Agent" distributed trojans and remote access malware. Over 60,000 social media followers were targeted by impersonation scams during the chaotic rebrand period.
...
## Why enterprises are banning ClawdBot

As a consequence, multiple enterprise organizations and industry analysts have taken firm positions against ClawdBot in corporate environments.
...
Whether you are actively running OpenClaw or concerned about unauthorized deployments, these steps will reduce your exposure.
...
### Enterprise detection and blocking

To identify unauthorized ClawdBot deployments in your organization:
...
If your organization decides to proceed after evaluating OpenClaw security risks, use it in a controlled capacity with a thorough [security assessment](https://sythelabs.com/process):
...
In addition, organizations subject to regulatory frameworks should carefully evaluate how ClawdBot usage affects their compliance posture. The tool's plaintext credential storage, unrestricted system access, and lack of audit logging may conflict with requirements under SOC 2, HIPAA, PCI DSS, and similar frameworks.

If employees deploy ClawdBot on systems that process regulated data, the unauthorized access and data handling could constitute a compliance violation. Review your organization's acceptable use policies and ensure they explicitly address agentic AI tools with system-level access.
...
Search for the directory paths listed in the detection section above (`~/.openclaw/`, `~/clawd/`, `~/.clawdbot/`). Monitor network traffic for connections on ports 3000, 18789, and 5353. Check endpoint detection logs for processes associated with OpenClaw. Finally, consider implementing application allowlisting to prevent unauthorized installations.
...
The broader lesson extends beyond this single tool. As agentic AI security risks grow alongside adoption, organizations need to evaluate these tools with the same rigor applied to any third-party software that requests privileged access. That means thorough security assessments, strict access controls, continuous monitoring, and clear policies for acceptable use.

If your organization is navigating the security implications of agentic AI tools, [reach out to our team](https://sythelabs.com/contact) for guidance on evaluating and securing these emerging technologies.
